    ...
    def read_files(self):
        ...
        with open(directory+file_name) as f:
            for line in f:
                ...
                cliente_general= General(name, dni, age, race, ondulado, n_ticket, validado)
        ...
        with open(directory+file_name) as f:
            for line in f:
                ...
                n_ticket=int(line.split(",")[5])
                validado=line.split(",")[6]
                cliente_vip= Vip(name, dni, age, race, ondulado, n_ticket, validado)
...
#MODULO 2
                    ...
                    if tipo=="G":
                        ...
                        if self.entradas[option].general==0:
                            ...
                            for i in range(int(num_entrada)):
                                ...
                                cliente= General(nombre,cedula,edad,carrera.race_name,False,0, False)
                        ...
                        if self.entradas[option].vip==0:
                            ...
                            for i in range(int(num_entrada)):
                                ...
                                if i != int(num_entrada) -1:
                                    print("Ingrese el siguiente usuario: \n")
                                cliente= Vip(nombre,cedula,edad,carrera.race_name,False,0, False)
                                clientes.append(cliente)
    ...
    Ids de los Asientos Vendidos: {asientos_string}""")
        ...
        with open(directory+file_name,"w") as f:
            for cliente_general in self.general:
                f.write(f"{cliente_general.name},{cliente_general.dni},{cliente_general.age},{cliente_general.race},{cliente_general.ondulado},{cliente_general.n_ticket},{cliente_general.validado},\n")
        
        file_name="ClientVIP.txt"
        with open(directory+file_name,"w") as f:
            for cliente_vip in self.vip:
                f.write(f"{cliente_vip.name},{cliente_vip.dni},{cliente_vip.age},{cliente_vip.race},{cliente_vip.ondulado},{cliente_vip.n_ticket},{cliente_vip.validado},\n")
    ...
    def get_actual_id(self):
        ...
        if len(self.general)==0:
            last_vip=self.vip[-1]
            return last_vip.n_ticket
        
        if len(self.vip)==0:
            last_general=self.general[-1]
            return last_general.n_ticket
...
#MODULO 3
    def attendance_ticket(self):
        print("GESTIÓN DE ASISTENCIAS A LAS CARRERAS")
        tipo=input("Ingrese su tipo de boleto, (V)ip o (G)eneral: ").capitalize
